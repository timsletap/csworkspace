...
public class RecursiveMethodsAT {
	...
	private static int maxValue(int[] arr, int i) {
		if(i < arr.length -1) {
	...
	private static int findSum(int[] a, int i) {
		if(i < a.length-1) {
	...
	private static int search(int[] arr, int lookFor, int i) {
		if(i < arr.length-1 && arr[i] != lookFor) {
			return search(arr, lookFor, i+1);
		} else if(arr[i] == lookFor){
			return i;
		} else {
			return -1;
		}
...
/**
	...
	private static int sumOfArray(int[][] array, int i, int j) {
		if(i < array.length) {
			if(j < array[i].length-1) {
				return array[i][j] + sumOfArray(array, i, j+1);	
			} else {
				return array[i][j] + sumOfArray(array, i+1, 0);
			}
		}
		return 0;
	}
...
/**
	 ...
	 * @param c
	 */
	public static void fillArray(char[][] array, char c) {
	 fillArray(array, c, 0, 0);
	}
	
	private static void fillArray(char[][] array, char c, int r, int t) {
		if(r < array.length) {
			if(t < array[r].length) {
				array[r][t] = c;
				 fillArray(array, c,r, t + 1);
			} else {
				 fillArray(array, c, r+1, 0);
			}
		}
	}

/**  13. Recursively finds the index number of lookFor in an array using BinarySearch * 
