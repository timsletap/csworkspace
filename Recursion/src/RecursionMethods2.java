...
public class RecursionMethods2 {
	...
	public static String removeVowels(String word) {
		if(word.length() == 0) {
			...
			if(temp.contains(word.substring(0,1))) {
				return ""+ removeVowels(word.substring(1));
			} else {
				 return word.substring(0, 1) + removeVowels(word.substring(1));
			}
		}
	}
	...
	public static double calcCompoundInterest(double principle, double rate, int years) {
		if(years == 0) {
			...
			return calcCompoundInterest(principle * (1 + rate/100), rate, years - 1);
		}
	}
	/*
	...
	public static double piOverFour(int y) {
		...
		return Math.pow(-1,  y + 1) / (2 * y - 1) + piOverFour(y-1); 
	...
	public static double squareRoot(double n, double prevGuess) {
		...
		if(n / prevGuess == prevGuess) {
			return prevGuess;
		} else {
			return squareRoot(n, (prevGuess + n / prevGuess) / 2);
		}
